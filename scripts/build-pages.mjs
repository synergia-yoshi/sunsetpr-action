...
const home = shell({
  ...
  <div>
    ...
    <p class="lead">Search ${database.entries.length} exact OpenAI, Anthropic, and Google Gemini model IDs. Then scan your repository read-only and turn eligible findings into a CI-verified draft repair PR.</p>
    <div class="actions">
  ...
  <aside class="proof" aria-label="Public verification">
    <strong>230 / 230</strong><p>labeled positive and negative synthetic cases; 100% recall and 0% false positives in the published regression suite.</p>
    <strong>5 refs · 6 files</strong><p>real GitHub App → Cloud Run → bot-created draft PR; generated compatibility test and customer CI passed.</p>
