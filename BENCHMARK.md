...
| --- | ---: | --- |
| Positive | 268 | 20 hand-designed SDK/config cases plus every one of the 124 lifecycle IDs in one TypeScript and one Python model-variable context |
| Negative | 268 | 20 hand-designed non-model cases plus every lifecycle ID in one TypeScript documentation context and one Python display-label context |
...
Real-repository evaluation must be reported separately from this synthetic corpus. SunsetPR does not combine the public demo result with these 536 labels to inflate a success rate.
